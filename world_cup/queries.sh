...
echo "$($PSQL "SELECT DISTINCT name 
              FROM teams 
              FULL JOIN games AS wi ON wi.winner_id = teams.team_id
              FULL JOIN games AS op ON op.opponent_id = teams.team_id
              WHERE (wi.year = 2014 AND wi.round = 'Eighth-Final') OR (op.year = 2014 AND op.round = 'Eighth-Final')
              ORDER BY name
              ;")"
...
echo "$($PSQL "SELECT year, teams.name 
              FROM games JOIN teams ON games.winner_id = teams.team_id
              WHERE round = 'Final'
              ORDER BY year, teams.name
              ;")"
